...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} body{{margin:0;color:var(--txt);font-family:"Noto Sans KR","Malgun Gothic",sans-serif;background:
    ...
    .head{{text-align:center;margin-bottom:8px;position:relative;overflow:visible}} .meta{{font-size:10px;color:#6f8299;letter-spacing:.16em}} h1{{margin:6px 0;font-size:34px;line-height:1.05;letter-spacing:.01em}} .sub{{font-size:14px;font-weight:700;color:#c7d6ea}}
    ...
    .qemoji{{font-size:30px;line-height:1}}
    ...
    .weekStrip{{display:flex;gap:10px;overflow-x:auto;padding:4px 2px 2px}}
    .weekCard{{min-width:186px;padding:12px;border-radius:14px;border:1px solid rgba(255,255,255,.12);background:rgba(15,23,42,.34)}}
    .weekHead{{display:flex;justify-content:space-between;align-items:flex-start;gap:8px}}
    .weekDay{{font-size:16px;font-weight:900;color:#f8fbff}}
    .weekDate{{margin-top:2px;font-size:12px;font-weight:800;color:#bfd2ea}}
    .weekIcon{{font-size:38px;line-height:1}}
    .weekState{{margin-top:8px;font-size:14px;font-weight:800;color:#dbe7f7}}
    .weekTemps{{margin-top:8px;font-size:24px;font-weight:900;color:#f8fbff}}
    .weekRain{{margin-top:6px;font-size:13px;font-weight:900;color:#fde68a}}
    .weekAlerts{{margin-top:8px;font-size:12px;font-weight:800;color:#fecaca;min-height:16px}}
    .weekActivities{{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}}
    .weekBest{{box-shadow:inset 0 0 0 1px rgba(251,191,36,.42);background:linear-gradient(180deg,rgba(251,191,36,.12),rgba(15,23,42,.34))}}
    ...
    .fun{{margin-top:8px;padding:18px 20px;border-radius:14px;background:linear-gradient(90deg,rgba(251,191,36,.18),rgba(236,72,153,.16));border:1px solid rgba(251,191,36,.42);font-size:24px;line-height:1.82;font-weight:900;letter-spacing:.035em;color:#fde68a;box-shadow:0 14px 34px rgba(124,45,18,.18);animation:pulseFun 2.6s ease-in-out infinite}}
    .funLabel{{display:block;margin-bottom:8px;font-size:14px;font-weight:900;letter-spacing:.18em;color:#fff1b3}}
    ...
    .microEmoji{{font-size:30px;display:inline-block;transform-origin:center;animation:briefingOrbit 2.8s ease-in-out infinite}}
    ...
    @keyframes briefingOrbit{{0%,100%{{transform:translateY(0) rotate(0) scale(1)}}20%{{transform:translateY(-3px) rotate(-10deg) scale(1.08)}}50%{{transform:translateY(-6px) rotate(8deg) scale(1.18)}}80%{{transform:translateY(-2px) rotate(-6deg) scale(1.06)}}}}
    ...
    @media (max-width:760px){{
      ...
      .weekCard{{min-width:168px}}
      .hourCard{{min-width:116px}}
      .summary{{font-size:15px;line-height:1.5}}
      .summaryTrek{{font-size:18px;line-height:1.4}}
      .fun{{font-size:20px;line-height:1.7;padding:16px 16px}}
      .funLabel{{font-size:13px}}
      .microEmoji{{font-size:26px}}
  ...
  <script>
    ...
    const spark = (arr, labels, color, mode='line') => {{
      ...
      if(mode==='bar'){{
        ...
        body += `<line x1='${{p}}' y1='${{h-p+1}}' x2='${{w-p}}' y2='${{h-p+1}}' stroke='${{color}}' stroke-width='1.8' style='opacity:.85;stroke-dasharray:420;stroke-dashoffset:420;animation:drawLine 1.1s ease forwards'/>`;
      }} else {{
        if(arr.length < 2){{
          ...
          body=`<polyline points='${{pts}}' fill='none' stroke='${{color}}' stroke-width='3' style='stroke-dasharray:520;stroke-dashoffset:520;animation:drawLine 1.2s ease forwards'/><circle r='4' fill='${{color}}' style='filter:drop-shadow(0 0 6px ${{color}})'><animateMotion dur='1.2s' fill='freeze' path='M${{pts.replace(/ /g,' L')}}'/></circle>`;
        }}
      }}
    ...
    async function fetchAll(loc){{
      ...
      f.searchParams.set('current_weather','true'); f.searchParams.set('forecast_days','7');
    ...
    function buildState(loc, forecast, air, selectedDate){{
      ...
      const weekly = (dly.time||[]).slice(0,7).map((d,i)=>{{
    ...
    function render(){{
      ...
      const rowsW = d.weekly.map(w=>`<div class='weekCard ${{w.best?'weekBest':''}}'><div class='weekHead'><div><div class='weekDay'>${{w.day}}</div><div class='weekDate'>${{w.dateText}} (${{w.dow}})</div></div><div class='weekIcon'>${{w.icon}}</div></div><div class='weekState'>${{w.state}}</div><div class='weekTemps'>${{w.high}}° / ${{w.low}}°</div><div class='weekRain' style='color:${{rainColor(w.rain)}}'>강수 ${{w.rain}}%</div><div class='weekAlerts'>${{w.alerts.join(' · ')||'주의사항 없음'}}</div><div class='weekActivities'><span class='activityBadge ${{w.trek?'activityOk':'activityNo'}}'><span class='e'>🥾</span>${{w.trek?'트레킹 가능':'트레킹 주의'}}</span><span class='activityBadge ${{w.run?'activityOk':'activityNo'}}'><span class='e'>🏃</span>${{w.run?'러닝 가능':'러닝 주의'}}</span>${{w.best?"<span class='activityBadge activityBest'><span class='e'>⭐</span>베스트</span>":''}}</div></div>`).join('');
      ...
      app.innerHTML = `
        ...
        <div class='tab' id='tw'><div class='card'><div class='miniCharts'><div class='tile'><div>최고기온 추세</div><div class='chart' id='c3'></div><div id='m3'></div></div><div class='tile'><div>강수확률 추세</div><div class='chart' id='c4'></div><div id='m4'></div></div></div><div class='weekStrip'>${{rowsW}}</div></div></div>
